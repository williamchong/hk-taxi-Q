...
| `P3-50` ⬜ | Penalties — the tip docked for the negative stuff (2026-09-25, the user's ask): a `Fare.Award` with NEGATIVE `hkd` on the same receipt, `CRASH` first (a hard stop against a wall or a building), later a kerb mounted at speed, a wrong-way spell (`WrongWayMonitor` already decides it), a red light when `Q77`'s heads light. The tip floors at zero (`FareSystem.tip_of`), the meter is never docked (`Q141`). The detector is the open half: a per-tick speed loss over a bar (`linear_velocity` delta on the physics tick, the drift's own `_apply_drift` cadence), or `VehicleBody3D` contact monitoring with the impulse read — measured on `skidpad.tscn` against a wall before a bar is chosen, and graded at more than one `--run-up` (`handling.md`). Prices in `skills.tres` (`crash_hkd`, `crash_min_kph`, a cooldown so one wall is one crash); the flash in `accent_negative`; a hurt face from `make_emote.py` | `verify_fares`: one crash docks `crash_hkd` from a live tip, a crash on an empty tip leaves it 0 and the receipt says `crash −5.0`, two hits inside the cooldown are one; a bar from both sides; `verify_hud` the strings; the user's drive |
