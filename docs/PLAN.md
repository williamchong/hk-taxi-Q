...
| `P3-16` | ✅ **Traffic signs from `DTAD_TS_ABV_PT` / `DTAD_TS_POLE_PT`** — done 2026-08-23 — **777 plates on 699 poles** as `signs.glb`: NO ENTRY, GIVE WAY, the mandatory turn discs, the turn prohibitions and the supplementary arrow plates, flat-shaded, standing on the poles TD surveyed. Text faces, parking / time-plate / vehicle-class signs and pictogram textures deliberately out of scope; signal heads stay in `P3-17`, and only sign-carrying poles ship — both the user's call | The whitelist is transcribed from TD's own **scanned** `TS` Index Plan sheets with `signCatalogue.json` as cross-check only, never authority (`Q59`); no rendered text (`Q42`) and **no texture ships**. 🔴 **The task changed on two measurements**: `DTAD_TS_ABV_PT` is the *abbreviation* layer and does not say where a sign is — **zero** of 3,276 points sit on a pole, p50 2.63 m away — so position comes from the pole through `GG_NAME`, which resolves **92.6%** to exactly one; and `ANGLE` is the MicroStation label rotation, flat against the road (p50 44.2°, 19.2% along / 18.1% across against 22.2% uniform), which `~/hk-traffic-sign-map` had already proved and reverted. So the facing is **derived** from host edge + kerb side + drive-on-left, and it is **ungraded** — there is no published subset to check it against, which is stated rather than papered over. The stage publishes counters that can see it fail: two closing partitions, `facing_away` **0**, `no_entry_with_flow` **0** (it read 117 of 253 before the one-way branch), and `no_entry_on_two_way` **7** as the surviving report-only second-source diff. ⚠️ The turn-restriction half of that diff is **owed, not done**. Dimensions are authored because the sheets are stamped NOT TO SCALE (`Q60`'s debt); candidates snap to level 0 (`Q15`); one primitive, one draw call, **no collider**; `city.json` bumped 14 → 15 and the manifest key optional, both sides one commit |
| `P3-20` | **Sign faces as a texture atlas — and the first texture in the bundle** — extracted at build time from TD's own `TS` Index Plan sheets, packed into one atlas in `game/assets/generated/`, sampled by `signs.gdshader` through a `TEXCOORD_0` the plates carry. Unlocks what `P3-16` had to refuse: `TS102`'s 讓, `TS182` CYCLES ONLY ×155, the pictogram families, and the ~2,364 text-faced plates — taking the layer from 582 plates toward the region's 3,276 | ⚠️ **This is a contract amendment, not a feature.** `mesh_contract.gd` fails any shader uniform holding a `Texture` and every verify tool routes through it, so the check, `ART_DESIGN.md`'s flat-shaded commitment and `PROGRESS.md`'s measured `Texture memory: 0` all move together and the decision gets its own `Q` record. 🔴 **Never commit the atlas** — it is derived from government drawings, so it is generated data under hard rule 7 and stays gitignored. The extraction is ported to **Python**, not run from `~/hk-traffic-sign-map`'s Node scripts (CLAUDE.md forbids node here); its approach is the reference and its `signCatalogue.json` stays cross-check only, never authority (`Q59`). ⚠️ **Its cardinal rule transfers: "a missed sign degrades to a dot — a *mislabelled* sign must never ship"**, so the code↔pictogram bind needs a human gate rather than an OCR result taken on trust. Grid detection is proven — 300 dpi raster, 23 vertical and 23 horizontal rules found on the `TS 101 - 205` sheet. Atlas size measured against the 128 MB budget, and the PCK delta isolated by a control export |
| `P3-21` | **Road lettering from `DTAD_RD_MARK_ANNO`** — the **274** published road annotations in region (`CENTRAL`, `九龍`, `KOW`, `LOON`; `FontName` `ENGINEERING` 181 / `chinese` 91, with `FontSize`, `Angle`, `CharacterWidth`) drawn as textured decals on the carriageway, gated on `P3-20`'s atlas | ⚠️ **Only the lettering, and that boundary is the whole task.** `/eval` returned **NO-GO** on texturing road markings generally: `TEXCOORD_0.x` is a *lane* coordinate, and markings survive the 1.6× widening precisely because an integer U is a lane boundary whatever the widening did to metres — a metric atlas UV reintroduces exactly what the current design solves. Nothing is removed either way, since the `TEXCOORD_1.x` codec still decides layout, and procedural edges with `fwidth` are sharper than a mipmapped texture at the grazing angles a road is always seen at. Lettering is the one part geometry cannot reach, the same argument as 讓 on a plate. Position and angle are **read**, never derived (`Q54`); the glyph set comes from the annotations actually published, not from a font shipped whole |
| `P3-17` | **Signal heads from `DTAD_TRAFFIC_LIGHT_PT`** — 913 published heads (aspect from `REFNAME`, facing from `ANGLE`) as static geometry on their poles. Signal *state* deliberately out of scope: no dataset publishes timing, and nothing consumes an aspect until `P3-3`'s traffic exists | Heads ship geometry-only and unlit — an invented cycle instructs and nothing obeys it; the named route for state is `P3-11d`'s `instance uniform` lamp-circuit pattern, wired in `B3` when traffic can stop at them; the aspect vocabulary is config data (hard rule 3); level-0 snap (`Q15`); one primitive, one draw call, **no collider**; `city.json` bumped and the manifest key optional, both sides one commit |
