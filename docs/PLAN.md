...
| `P3-17` | ✅ **Signal heads from `DTAD_TRAFFIC_LIGHT_PT`** — done 2026-08-26 — **415 heads standing for 681 published aspects** as `signals.glb`, static and unlit, 26,560 triangles. Signal *state* deliberately out of scope: no dataset publishes timing, and nothing consumes an aspect until `P3-3`'s traffic exists | 🔴 **The task changed on two measurements and the geometry on a third.** **913 is not 913 heads**: 46 codes, of which 51 features are bollards, push buttons and wig-wags, and `REFNAME` has **no published domain** — no Index Plan sheet defines it, both Miscellaneous Details sheets were rendered and read, `signCatalogue.json` is `TS`-only. So the code is a **gate**, never a look, and `drawn_by_code`/`refused_by_code` publish the whole vocabulary because nothing downstream can grade a spelling rule (`Q76`). `M<n>` ×19 is refused as undecoded — one word to reverse. **`ANGLE` carries no facing**, re-measured rather than assumed (p50 44.3° off the host axis, 21.3% along / 19.3% across against 22.2% uniform), so the facing is `signs.facing_from_side`'s and is **ungraded** (`Q62`) — the evidence is an A/B render. 🔴 **And an assembly is not a stack**: there is no `GG_NAME`, 470 of 913 points are coincident, and the first build stacked them into **8.53 m masts** — caught by *looking*, with both partitions closed and `check.sh` green. One head per assembly now, with `assembly_size` publishing the collapse. 72.7% surveyed inside the ribbon, so `Q60`'s registration at a third layer; level-0 snap (`Q15`); one primitive, one draw call, **no collider**; shares `signs.gdshader` (`Q61`/`Q71`) with `sheeting_glow` **0**, because a lit-looking unlit lens is an instruction this game cannot honour. `city.json` **17 → 18**, both sides one commit. PCK 42,884,244 → 43,483,032 B (+1.40%) |
