...
| `P3-20` | 🟡 **Sign faces as a texture atlas — RESCOPED by `Q65` from 193 cells to about 2, and the user's call is owed before it is built.** The scope cut removed the ~2,100 time, parking, zone and vehicle-class plates the atlas was sized against, so what is left needing lettering is `TS102`'s 讓 ×**74** — which ships today as a bare red triangle and is how the user found the defect — and `TS101` STOP / 停 ×**18**. Two cells, both nameable, from two named sheets | ✅ **The contract half is done**: `Q63` landed 2026-08-23, `mesh_contract.gd` admits a texture only where a call site declares a pixel budget, `verify_mesh_contract.gd` asserts the refusals on stub meshes and needs no built region, and `Texture memory` is still **0**. 🔴 **Most of the machinery this task was scoped around is gone with the cells**: no OCR, no contact sheet, no `--propose`/`--commit` gate and no grid sweep are needed to find two cells you can name, so `tools/sign_atlas_survey.py` is **not** owed. ⚠️ **Build it as its own primitive.** `mesh.py` refuses to merge meshes with and without UVs, so `TEXCOORD_0` on the existing asset costs all **34,564** vertices a channel — 276,512 B raw, ~138 KB of PCK, **+0.32%** of the bundle — paid by 706 plates and 541 posts to serve 92. A separate `signs_text.glb`, as `railings.glb` already ships three, keeps the untextured majority byte-identical and scopes the declaration to one surface, for one draw call. ⚠️ **NO-GO is equally defensible and is a taste call, not an engineering one** — an octagon and a red-bordered triangle already say stop and give way. The PDF route is proven and its dependencies are in: `pdfminer.six` reads the sheets' vector rules in points, `pypdfium2` renders the cells. `Q65` |
...
| `P3-22` | ✅ **The direction signs that were refused for a reason that never applied** — `TS414` PERMANENT SHARP DEVIATION ×**61**, `TS735` ARROWS ×**20**, `TS615` NO THROUGH ROAD ×**14**, `TS589` chevron marker ×**10**. **105 plates, zero textures**, on about three new `draw` primitives | 🔴 **This is `Q64` at a second layer**: four codes whose meaning is their *shape*, refused with the text families they do not belong to. Every one read off TD's own sheet — `TC 210`, `TC 408`, `TC 310` — and never off `signCatalogue.json`, which is the rule `Q64` exists to insist on. ⚠️ **Composite glyphs are baked as one `draw` word**, not given a per-layer offset in the face schema: `_bent_arrow` and `_u_turn_arrow` are the precedent, and a schema change would be the larger claim. ⚠️ `TS735` is the cheap one — a double-headed arrow on the `rect_wide` plate its `TS733`/`TS734` siblings already use. ⚠️ **Bend and narrows warnings (`TS401`–`TS418`) are deliberately NOT here**: 53 plates across 13 codes, at most 8 each, for 13 new primitives — the worst effort-per-plate in the estate, and `Q65` records the refusal. Same bar as `P3-16`: partitions close, `facing_away` **0**, one primitive, one draw call, no collider. `Q65` |
