...
| `P4-5` | Perf pass — 23.3% more drivable area, and the streamer's bands were tuned without it | 60fps on the device floor with the elevated network resident. 🔴 **And now a second operating point (`Q120`)**: the shipped `streaming.tres` pair (LOD0 <250 m, unload 400 m) is **108% of the 300k triangle budget** at the worst camera in `mong_kok` against 71% in `wan_chai`, so this task no longer tunes for one region's density. ⚠️ **The cheapest lever is not a distance** — LOD1 saves only 51–62% (L1/L0 0.38–0.49) while carrying 13.6 of 22.3 resident tiles, so a harsher LOD1 or a third tier is ETL-side and costs no draw calls. ⚠️ Pull-in distance is `Q26`'s look decision as well as a hitch budget |
...
| `P5-6` | **Roads chunked by tile** — the measured ribbon cut at stations on the 150 m grid, caps whole to one tile, one `-col` per chunk; the streamer gains a second tile content class | The union of chunks reproduces today's `roads.glb` in positions, normals, colours, indices and both `TEXCOORD`s **up to the duplicated cut stations, counted**; seam gap **0.000 m** at every cut, measured. `verify_road_surface.gd` per chunk; `paint_clearance`, `kerbside_error`, `overhang`, `deck_margin` loop over chunks and reproduce their tables. Draw-call delta on the driving route pasted against the region-wide 1. `city.json` schema bumped. ✅ **The cost is now measured and bounded (`Q120`)**: library mesh counts are identical to the mesh across four regions (9/9/12/10; `lamps.glb` byte-identical at 3,568 B) while placements swing 4–8×, so **draw calls are region-invariant** and this task's +20–25 is paid **once for the territory**, not per km². ✅ The premise holds everywhere: `roadsurface` spans **~1,660 m in three of four** regions, so one un-cullable, un-streamable instance is structural and not a Wan Chai artefact. 🚫 **Not a road kit** — `Q115` refused that on geometry and `Q120` refuses it again on payoff (road geometry is **2.4%** of `mong_kok`'s bytes, and the share *falls* as density rises); this splits the same measured geometry and invents nothing. ✅ **Unblocked 2026-09-07**: the second region exists as a built bundle (`Q120`), which is what "starts when the second region starts" waited for. ⚠️ **Runs before `P5-7`**, because its acceptance is a byte-identity against today's `roads.glb` and the join moves the graph under it; proved inert on Wan Chai alone first, then re-run once the join lands |
