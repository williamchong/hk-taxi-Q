...
names — **134 files and 30.8 MB** for Wan Chai, exporting to a **26.3 MB** PCK with tile collision.
  ...
  "landmarks": [
    {
      ...
      "transform": { "pos": [102.5, 4.0, 84.0], "rot_y_deg": 6.4 },
...
from config plus one CRS conversion, which is why it is not a stage of its own. The manifest
names it under the `landmarks` key; `game/scripts/city/landmarks.gd` places the models, and
`generated_landmarks.gd` is the locator.
...
The `.glb` assets are **committed** under `game/assets/authored/landmarks/` (CC BY-SA 4.0,
generated by `tools/make_landmark.py`); they are not build output, so `shipped()` never lists
them and `sync_generated.sh` never touches them.
