...
| `lanes` | ⚠️ **Not published *as an attribute*.** Road Network v2 carries no lane field in any layer. Authored per road class in city config, keyed on speed limit. ⚠️ **Derivable, though** — Traffic Aids Drawings publishes the lane lines themselves (`RM1101`/`RM1102`) between published carriageway edges (`RM1109`), so this is authored for want of a counter rather than for want of data (`Q57`) |
| `width_m` | Derived from `lanes`, then hand-tuned upward for playability. ⚠️ **A real carriageway width *is* published** — iB1000 `CartoTransLine` `RM`, "Road margin", on sheets the build already downloads (`Q57`). This field is not it |
...
| `scripts/city/*_preview.gd` | Dev previews — one per drawn class: `tile`, `road_surface`, `road`, `fare`, `tramway`, `arrows`, `boxjunctions`, `roadmarks`, `railings`, `signs`, `lamps`. 🔴 **Adding a drawn layer means adding its node to `city_drive.tscn` AND `city_preview.tscn`** — `roadmarks` had everything else and no node at all (`Q73`), and `lamps` shipped into the preview scene only, so it was correct, verified and invisible in the game (`Q82`). Two scenes, both times, and no tool can see either — `roadmarks` had everything else and no node, and nothing caught it (see Checks). They instantiate what the manifest names so a layer can be looked at on its own. **Not performance measurements** |
