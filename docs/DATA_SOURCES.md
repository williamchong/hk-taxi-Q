...
| **RM1001 DOUBLE LINES is published and not drawn** | `DTAD_RD_MARK_LINE` carries **19,308 m in region**, second only to RM1109's 25,204 m and ~90x the 211 transverse parts `roadmarks.py` draws; `roadmarks.json`'s `refused_m_by_code` publishes it. Its sheet row is `LINE WIDTH = 150, LINES SPACING = 100` (`CT174/51-5(1)F`), and ⚠️ **LINES SPACING is the clear gap, not a pitch** | `Q117` draws the centre line at those proportions from an **inferred** opposed-pair join, so the shape is sourced and the placement is not — a `Q54` debit, because a double continuous white line instructs *no overtaking*. Drawing the surveyed geometry instead is blocked on `roadmarks.py` hosting by **transversality**: RM1001 runs *along* the road, so the rule that makes stop lines correct refuses every metre of it |
