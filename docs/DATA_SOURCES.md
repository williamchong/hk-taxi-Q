...
| `StreetCentreLines`, `STREETTYPE` | 794 | Carries `ST_CODE`, the same street code `NSR` joins on — a second key, not a second graph. **In use since 2026-09-24** as the street hierarchy (`roads.street_class`): the domain is in every sheet's gdb — `HIG` Highway, `TUN` Tunnel, `ROB` Road Bridge, `SLR` Slip Road, `MAR` Main Road, `SER` Secondary Road, `TRA` Track, `RER` Restricted Road. 25 of 585 codes carry two types (Canal Road East/West, Morrison Hill, Tung Lo Wan Road), so an edge takes its nearest same-code segment, not the majority. ⚠️ Lockhart, Jaffe and Harbour Road are `SER` |
...
| **`Shoreline`, `SHORELINETYPE`** | — | Wan Chai frame 67 lines | **The harbour** — on the minimap since 2026-09-24, in the world since 2026-09-25 (`pipeline/basemap.py`, `water.glb`; the tile stage sinks the ground under it). Domain in the gdb: `SWA` Sea Wall, `HWM` High Water Mark, `BRE` Breakwater. ⚠️ No sea POLYGON anywhere in the sheets — `HydroPolygon` is culverts, ponds and streams — so the sea is cut from the frame along these. ⚠️ The lines do not close: pier and shelter outlines dangle at dozens of sub-metre joins and pier ends stop ~35 m short, so they are thickened by `seal_m` (40 m) before the cut. ⚠️ The region's own sheets stop 76 m north of Wan Chai; the north row (`11-SW-9B`, `10A`, `10B`, `11-SE-6A`, `6B`, `7A`) is fetched by `tiled_sources.topography.fetch_margin_m` |
| **`Site`, `SITECODE`** | — | Wan Chai frame 86 open-space polygons | **The minimap's parks** since 2026-09-24, **and the world's** since 2026-09-25: the tile stage paints the ground inside them `materials.park_grass` (`Q144`). Domain: `PAR` Park / Garden, `PLA` Playground, `SOA` Sitting-out Area, `SGR` Sports Ground, `PRO` Promenade. Victoria Park is one `PAR` of 188,001 m². `SITESTYPE` is an integer category — key on `SITECODE` |
