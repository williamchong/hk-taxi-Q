...
| **`Site`, `SITECODE`** | — | Wan Chai frame 86 open-space polygons | **The minimap's parks** since 2026-09-24. Domain: `PAR` Park / Garden, `PLA` Playground, `SOA` Sitting-out Area, `SGR` Sports Ground, `PRO` Promenade. Victoria Park is one `PAR` of 188,001 m². `SITESTYPE` is an integer category — key on `SITECODE` |
