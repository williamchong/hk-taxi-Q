...
| **Lane counts do not exist *in this dataset*** | no lane attribute in any field of any layer | `roadgraph.json`'s `lanes` is authored policy keyed on speed limit, not published data. ⚠️ **`Q57` narrowed this claim on 2026-08-20 without refuting it.** No source in the estate publishes a lane *count*; Traffic Aids Drawings publishes the lane *lines* — `RM1101`/`RM1102` LANE LINES (212 features in region), `RM1103` CENTRE LINE, `RM1109` EDGE OF CARRIAGEWAY (317) — so a count is derivable per cross-section from geometry. It is authored today because nobody has counted them, not because the city withholds the number. 🔴 **Narrowed again on 2026-08-28 by `Q94`, and this time from a layer already in the bundle**: a *row of turn arrows across a carriageway is a lane count*, stated by the publisher and read by a shipped stage. `DTAD_RD_MARK_SYM_PT` puts three symbols side by side at one station on STEWART ROAD `e505` — left | left-or-right | right at offsets +2.96 / −0.32 / −3.59 — which is a three-lane approach written down. Grouped by along-edge station and split at 1.6 m, **31 of the 306 arrow-carrying edges imply more lanes than the graph gives them**, nine of them four against two. It is a *sparser* source than the lane lines — only 306 edges carry arrows — but it needs no new geometry and no new fetch, and it is what `Q94` measured the cost of ignoring. ✅ **In use as a lane count since 2026-08-29**: `pipeline/carriageway.py` reads the same layer at the roads stage and lets a row resolve a TPDM bracket that names more than one integer, publishing `lanes_source: arrows` on **57** edges. 🔴 **A row of ONE arrow is refused** — the row counts *painted* lanes so it is a lower bound, and 81 edges state a single arrow, 26 of them on a bracket of one lane. ⚠️ So this row's "lane counts do not exist" still stands for the *estate*: what exists is a lower bound on 306 edges, not a count on 737 |
| **RM1001 DOUBLE LINES, drawn from the survey since `Q118`** | `DTAD_RD_MARK_LINE` carries **19,308 m in region**, second only to RM1109's 25,204 m and **11.5x** the 1,673 m of transverse marking `roadmarks.py` draws today (211 parts); `roadmarks.json`'s `refused_m_by_code` publishes it. Its sheet row is `LINE WIDTH = 150, LINES SPACING = 100` (`CT174/51-5(1)F`), and ⚠️ **LINES SPACING is the clear gap, not a pitch** | `Q117` draws the centre line at those proportions from an **inferred** opposed-pair join, so the shape is sourced and the placement is not — a `Q54` debit, because a double continuous white line instructs *no overtaking*. `Q118` draws it: `RoadMark.axis` gives the stage a second host rule, and **5,745 m of the 19,308 is at grade** — the rest is on structure, mostly the Central-Wan Chai Bypass. **91 parts / 3,416 m** ship. ⚠️ **And since `Q125` the inferred join is drawn too, as this same mark** — `road_marks.opposed_join_mark` — along the **1,974 m** of opposed pair no surveyed line covers (13 of 95 pairs are covered end to end, 24 partly, **58 not at all**); the shape stays TD's and only the placement is inferred. ⚠️ **WHITE is read off the Description column**, not inferred: RM1040-49 on the same sheet are yellow. ⚠️ RM1002/RM1003 break one line of the pair (1000 MARK / 5000 GAP) and RM1004 publishes `LINES SPACING = VARIABLE`, neither of which a config row can say. 🔴 **"Absent" was WRONG and is corrected (`Q132`, 2026-09-19)**: that was read off this layer alone, and TD files the broken half of the family in the sister layer `DTAD_RD_MARK_LINE_C` — **2,046 m of RM1002 and 2,165 m of RM1003 at grade in Wan Chai**, 386 m of RM1002 in Causeway Bay. RM1004 is 40.7 m here and 132 m there |
| Dual carriageways are **opposed one-way pairs** | 6 places where two one-way edges share both endpoints in opposite directions, **1.96–3.85 m** apart; three of them are Lockhart Road. ✅ **The lower bound is measured since 2026-08-29**: pairing geometrically instead of by shared endpoints, **110 of 352** one-way edges with a median find an anti-parallel partner that finds them back (`Q95`). 🔴 **And the pipeline stopped using the endpoint rule on 2026-09-06** — `surface.py` pairs geometrically too, at **49 pairs / 3,958 m** against the six's 564 m, because those 609 unpaired edges were drawing two overlapping ribbons with nothing between the flows and it was reported from the driving seat (`Q117`) | Lockhart Road is two-way *modelled as two one-ways*. A **lower bound** — carriageways that do not share both endpoints are not counted. ⚠️ **The six are the only ground truth this repo has for a pairing rule, and they are what caught the first one**: capping the partner search at the station's own far ray missed all three Lockhart pairs, because their spans (7.06–8.08 m) stop short of a partner centreline 6.82 m away. 5 of 6 recover under the ray cap; the sixth is 9.0 m long and has no station clear of a node |
...
| `DTAD_RD_MARK_LINE_C` | 1,413 features | The other marking half: `RM1104` warning line 409, `RM1101` lane lines 212, `RM1054` angled parking bays 169, `RM1007` bus-lane continuation 136. ✅ **In the build since `P3-34` (2026-09-19, `Q132`)** — `road_marks.more_layers` reads it beside `DTAD_RD_MARK_LINE`, and `roadmarks.py` draws `RM1101`/`RM1102` lane lines, `RM1103` centre lines, `RM1104`/`RM1105` warning lines and the `RM1002`/`RM1003` broken double whites: **24.3 km** drawn in Wan Chai, 5.0 km in Causeway Bay. ⚠️ `RULEID` is the one cartographic column that carries meaning: `RM1002` is rule 2 and `RM1003` rule 3 on every feature, which is what makes LEFT/RIGHT the digitised direction's. ✅ **Read by `tools/width_evidence.py` since `Q127`** through `carriageway_survey.lane_lines` (`RM1002`/`RM1003`/`RM1101`-`RM1104` here, `RM1001` from `DTAD_RD_MARK_LINE`): the spacing between lines abreast agrees with the turn arrows' own spacing to p50 **0.29 m**, but a lane *count* read off them over-counts on 51 of 75 edges, because the other carriageway's lines lie within reach |
...
Standards Division, which gives every `RM` code its marking, description and dimensions. ⚠️ **`RM1101` and up are a
SECOND sheet** — `Index Plan/(RM 1101 - 1180).pdf`, drawing `CT174/51-5(2)G` — read at 600 dpi on
2026-09-19 (`Q132`) with `RM1002`-`RM1004` off the first; both sets of rows below are verbatim:
...
| `RM1070` (TC 801) | **ZEBRA CROSSING — WHITE** | width 500-700, spacing 500-700, length not less than 2500 | stripes |
| `RM1076` (TC 806) | **LIGHT SIGNAL CROSSING YELLOW STRIPED MARKINGS — YELLOW** | width 250-350, space between edge of carriageway & 1st marking 500-1300, other spacing 600-800. ⚠️ the survey draws these 0.6 m wide, not 250-350 | stripes |
| `RM1071` (TC 802) | ZEBRA CROSSING GIVE WAY LINE — WHITE | line width 200, module 500 mark / 500 gap | — |
| `RM1072` / `RM1073` (TC 802) | ZIG ZAG MARK — WHITE (left hand & centre / right hand & centre & terminal) | line width 100, module 2000 mark / 150 gap. Surveyed as `ZIGZAGL` / `ZIGZAGR` in `DTAD_RD_MARK_LINE_C`; read as the evidence a crossing is a zebra | — |
| `RM1035` / `RM1036` (TC 512) | PROHIBITORY CHEVRON — WHITE | line width 150, module continuous, **chevron width 900**, distance between chevrons 2000 | — |
| `RM1037` (TC 513) | PROHIBITORY HATCHED TRAFFIC ISLAND MARKING — WHITE | line width 150, module continuous, distance between hatching not greater than 3000 | — |
...
| `RM1002` (TC 502) | DOUBLE LINES — WHITE | line width 150, lines spacing 100, **left line continuous, right line 1000 mark / 5000 gap** | **double, one broken** |
| `RM1003` (TC 502) | DOUBLE LINES — WHITE | line width 150, lines spacing 100, **left line 1000 mark / 5000 gap, right line continuous** | **double, one broken** |
| `RM1004` (TC 503) | CONTINUOUS DOUBLE LINES WITH HATCHING — WHITE | line width 150, **lines spacing VARIABLE**, both continuous, hatching ≯ 3000 apart | — |
| `RM1101` (TC 601) | LANE LINES — WHITE | line width 100, **1000 mark / 5000 gap** | **single, broken** |
| `RM1102` (TC 601) | LANE LINES — WHITE | line width 100, **2000 mark / 7000 gap** | **single, broken** |
| `RM1103` (TC 602) | **CENTRE LINE — WHITE** | line width 100, **3000 mark / 5000 gap** | **single, broken** |
| `RM1104` (TC 603) | **WARNING LINE — WHITE** | line width 100, **4000 mark / 2000 gap** | **single, broken** |
| `RM1105` (TC 603) | WARNING LINE — WHITE | line width 100, **6000 mark / 3000 gap** | **single, broken** |
| `RM1106` (TC 605) | JUNCTION EDGE LINE — WHITE | line width 100, 600 mark / 300 gap | single, broken |
| `RM1107` (TC 606) | MERGING/DIVERGING LANES, LAYBY, BUS STOP, PASSING PLACE EDGE LINE — WHITE | line width 100, 1000 mark / 1000 gap | single, broken |
| `RM1108` (TC 607) | EDGE OF CARRIAGEWAY — WHITE | line width 100, 1000 mark / 3500 gap | single, broken |
| `RM1109` (TC 607) | EDGE OF CARRIAGEWAY — WHITE | line width 100, continuous | single |
