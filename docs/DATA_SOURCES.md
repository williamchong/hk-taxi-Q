...
| **`NSR` is the kerbside yellow lines, and it was read past once** | **579 features / 44,220 m** in region, **kerb-referenced** (median **2.92 m** off the nearest centreline, 0% on it); `TIME_ZONE` + `REMARKS` separate an all-day restriction from posted hours, i.e. double yellow from single. `ONSTREETPARK` carries **607** bays as the complement | `P3-12` painted a double yellow on every kerb and recorded it as unsourceable. It is not. Ingesting it needs linear referencing — only **78%** of NSR parts agree on their own side across their length, **49%** span more than one centreline feature (`Q53`) |
