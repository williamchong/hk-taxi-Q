...
resolves only if the region grows east (`Q6`). ✅ **And the 6 clamped ramp touchdowns are ramped**
(`Q90`) — they were the rest of the residual and stayed invisible while the step was quoted as a
median. ⚠️ **What is left is one node**: 269, `MARSH ROAD`, where the deck ends in a 0.65 m vertical
face short of the node and the missing descent belongs to the level-0 `e466` beyond it — this
question's own attribute flip, at the one node where no per-edge sampler can reach it.
...
🔴 **Every peak-slip and dwell figure in this question is SUPERSEDED**, and by three changes rather
than one: `Q86` added the yaw burst, `Q87` and `Q88` gave both dials speed terms, and `Q89` added a
third envelope below the knee that 63 km/h sits inside. The shipped drift at 63 km/h is now
**69.8° / 0.87 s**. What stands here is the *method* — sweep this dial at 0.002, grade it on
`secs>thr` and never on peak — and the numbers are the car of 2026-08-27.
...
### Four things the slide does that no column reports

Measured while the assist was being tuned, and recorded because each is a fact about the *mechanism*
rather than about a dial, so none of them moves when a dial does.

🔴 **The slide self-terminates after about 1.8 s, so `exit` is measuring two seconds of dead car.**
Slip crosses `drift_slip_threshold_deg` at **1.17 s**, peaks 30–41° at ~1.5 s, and is back under the
bar by **2.0 s** — with the throttle held down the whole time. ⚠️ **Slip is not monotonic**, which is
the trap: reading dwell alone suggests the bar is crossed late in a 4 s run, and it is crossed early
and then lost. The `exit kph` figures every table above quotes therefore include ~2 s of a car that
has already stopped drifting and is simply slow.

🔴 **Lifting the throttle cancels the drift outright, and re-applying does not recover it.** Held
throttle peaks 21.8°; released at the moment of engagement peaks **7.3°** and never crosses the bar;
re-applied at 1.0 s reaches **4.0°**. This is `Q50`'s isotropy again from the other side — the rear
axle's drive is the same budget as its grip, so coasting hands the tail its grip back. ⚠️ It means
the genre's standard drift entry, *lift then flick*, does not exist in this model, and no dial here
reaches it.

🔴 **On a 90° corner the gripping turn beats the drift on every measure except line.** Grip: 63.0 →
**63.4 kph** (it gains), 1.10 s, 19.6 m. Drift: 52.8 kph, 0.95 s, 16.3 m, and it saturates past about
0.7 s of held button. **A drift buys line and time, never speed** — so any scoring or route design
that assumes drifting is the fast way round is assuming something false about this model.

🔴 **There is no sustained drift equilibrium at any counter-steer timing.** Tested with the counter
never applied and at 0.6, 0.9 and 1.2 s, the car has exactly two stable states: a **gripping circle**
(~2° slip, 53 kph, −522° of yaw over 6 s) or a **spin**. Nothing in between holds. ⚠️ **This is the
direct answer to "can it hold a Forza/Initial D line round a roundabout": not in this model**, and it
is a property of the friction mechanism rather than of the assist, so `Q86`'s burst does not change
it either.
...
- ✅ **The decaying torque is built and shipped** — `Q86`, which fixes the tap and supersedes the
  constant-torque table above.
...
## `Q86` — The tap needed the torque spent early, and the dwell was never the assist's to buy

**Status.** ✅ Closed 2026-08-27 · **Owner.** `handling.tres` → `vehicle_controller.gd` ·
extends `Q85`

**Claim.** A drift button that does nothing on a tap is not a fun button, and `Q85` left it that way
at every constant torque. Fixed by decaying the yaw torque from a peak toward a sustain over
`drift_yaw_decay_s`, timed from the press. **The tap goes from 2.4° to 16.0° of peak slip and from
0.00 s to 0.23 s above the threshold, and the held drift gets better at the same time.**

⚠️ **This was built because drift is a feel mechanic, not because it scores.** The prior
recommendation was to stop work until `P3-2b`'s scoring gave the drift a consumer; the user's
correction — *"drift is a fun factor not just because of score"* — is right, and `GAME_DESIGN.md`'s
**Target feel** table already said so. Recorded because the wrong framing was load-bearing for a
"stop working on this" conclusion, and because the instrument built in `Q84` (`secs>thr`) is what
made scoring feel like the point.

### Why decay, in one line of arithmetic

Torque × time is rotation, so a 0.5 s tap collects **one-eighth** of a 4 s hold's angular impulse.
One constant cannot serve both, and `Q85` measured exactly that: 1000 N⋅m gives a 42.1° hold and a
dead 2.4° tap; 5000 gives a live 27.0° tap and a **162.9° spin** of a hold. Spending the budget early
hands the tap the whole burst and leaves the hold a sustain it survives. At the shipped 0.8 s decay
the tap-to-hold impulse ratio is about **1.2:1** where a constant torque made it 8:1.

🔴 **The decay runs on TIME and must never be made to run on measured slip.** Backing the torque off
as the angle opens closes the loop, and "slip above the threshold" degrades into "the dial said so" —
`Q72`'s tautology through the side door, and the exact failure `drift_yaw_torque_nm`'s own note
refuses. On time it stays open-loop: the tyres still get to argue, so the angle is an outcome and
`secs>thr` keeps its meaning.

### 🔴 The assist cannot buy dwell, at any setting of any of its three dials

The single most useful result here, and it is a negative one. `secs>thr` was flat across **three**
independent sweeps:

| swept | range | `secs>thr` across the range |
|---|---|---|
| `drift_yaw_decay_s` | 0.4 → 2.0 s | 0.78 → 0.83 |
| `drift_yaw_sustain` | 0.0 → 0.3 | 0.85 → 0.82 |
| `drift_yaw_torque_nm` | 5000 → 11000 | 0.78 → 0.80 |

Over those same sweeps peak slip ran 40° → 130°. **The yaw dials buy angle and cost speed; they do
not buy time above the bar.** Dwell is bounded by the friction mechanism — `Q85` measured the slide
self-terminating at ~1.8 s — and `drift_rear_grip_scale` tops out at 0.85 s too (`Q84`). ⚠️ **So
these three dials must be graded on peak slip and on exit speed, never on `secs>thr`**, which is the
opposite of the rule `Q84` wrote for the grip dial and is why both rules have to be stated per dial
rather than once for "the drift".

### The pick

Swept at `drift_yaw_decay_s` 0.8 and `drift_yaw_sustain` 0.0:

| peak N⋅m | tap peak | tap `secs>thr` | tap exit | hold peak | hold `secs>thr` | hold exit |
|---|---|---|---|---|---|---|
| 5000 | 9.2° | 0.00 | 61.48 | 40.0° | 0.78 | 42.61 |
| **7000 (shipped)** | **16.0°** | **0.23** | **57.95** | **51.1°** | **0.82** | **41.29** |
| 9000 | 23.9° | 0.47 | 53.82 | 65.7° | 0.85 | 39.95 |
| 11000 | 33.2° | 0.58 | 50.18 | 87.1° | 0.80 | 38.63 |

✅ **7000 is better than the shipped baseline in every column** — tap 2.4° → 16.0°, hold dwell 0.78 →
0.82, and exit speed 40.96 → **41.29**, so for once the drift got better without buying it with
speed. ⚠️ **9000 was the candidate and was rejected by driving it.** On the skidpad its 65.7° hold
looks like a healthy angle; on Expo Drive it rotates the car **086° → 219°** and parks it against the
railing across the carriageway. The skidpad cannot see that, because an open pad has no far kerb.

⚠️ `drift_yaw_sustain` ships at **0.0**, which makes the shipped drift a pure kick that friction then
eats. It measured best on peak, dwell and exit at once, but 0.0 is a value that makes its own dial
dead, and the case for raising it — holding an angle through a corner longer than the skidpad's — is
a feel judgement that `P0-3b`'s handset has to make. It is kept as a dial for that reason.

### Control: the change reaches only the drift branch

`corner`, `brake` and `coast` are **byte-identical** before and after (2.0°/70.0 m, 8.75 m/s²/17.0 m,
1.84 m/s²/65.2 m). Worth stating because three of the five rows moving would have meant the burst was
leaking into the ordinary drive model.

### `tools/skidpad_ablation.gd` — the sweep is no longer one hardwired field

`--drift-grip` became `--sweep=<field>=<v1,v2,…>` over any profile float, with `--drift-grip` kept as
an alias. ⚠️ **The alternative was a shell loop editing `handling.tres`, and that loop has already
misfired once** — a bad `set --` blanked the field it was sweeping and published a table of all-zero
rows that read like a finding. Three yaw dials made a hand-edited grid the likeliest way to publish a
wrong number, so the hazard was removed rather than repeated.

⚠️ **A non-`drift_`-prefixed field now re-runs all five manoeuvres.** Only `drift` and `tap` reach
`_apply_drift`, so sweeping a drift dial correctly skips the other three; sweeping `engine_force`
would move all five, and reprinting the first value's rows under later labels is precisely the
...
- ⬜ **All three values are desk picks.** They were chosen off a skidpad grid and one drive; the
...
🔴 **The 63 km/h figures above are SUPERSEDED by `Q89`**, which added a third speed envelope below
the knee. 63 sits inside it, so the shipped drift there is now **69.8° / 0.87 s** and the tap
**20.5° / 0.40**. What is written above stands as what this change did to the car of the time.
...
which is what makes every other column comparable across rows — and a dial whose effect depends on
speed cannot be graded at one speed at all. `--run-up=<seconds>` now varies it: 4 s → 63.02, 6 s →
86.36, 8 s → 105.47 km/h. ⚠️ Rows are comparable only *within* one run-up; `entry kph` is the number
to quote, because the run-up is open-loop.

### The fix, measured

| entry km/h | run | before | after |
|---|---|---|---|
| 63.02 | drift | 51.1° / 0.82 s | **51.1° / 0.82 s** — identical |
| 63.02 | tap | 16.0° / 0.23 s | **16.0° / 0.23 s** — identical |
| 86.36 | drift | 165.0° (spin) | 103.2° |
| 86.36 | tap | 54.8° | **8.7°** |
| 105.47 | drift | 165.4° (spin) | 165.2° |
...
⚠️ Mirrors `_update_steering`'s speed taper, which solves the same shape of problem — an input
...
| entry km/h | peak slip, assist off |
|---|---|
| 63.02 | 21.8° |
| 86.36 | **95.2°** |
| 105.47 | **165.2°** |

**`drift_rear_grip_scale` spins the car at speed on its own**, and it has no speed term of any kind.
At 86 km/h even **1000 N⋅m** of assist reads 121.5°, and at 105 even **500** reads 165.2° — which is
...
- ⚠️ **The fade band is narrow (20 km/h) and linear.** Nothing has looked at whether the withdrawal
  is *felt* as the button going dead mid-corner, which is a frame question, not a table one.
...
too soon. But it meant the honest usable band was roughly **60–100 km/h**, not "everything below
100", and the bottom of it had never been measured — three consecutive changes verified "the design
speed is byte-identical" without ever asking what happens under it.

✅ **Fixed by `Q89` the same day**, which found the cut has to *deepen* below the knee and that a
taper tracking live speed there is unstable. The band is now **34–86 km/h**, and every figure in this
section describes the car before that change.

**What is not settled here.**

- ✅ **The bottom of the band was the next question, and `Q89` answered it.** Typical city driving is
  40–60 km/h, so the drift was least available exactly where it is most used; it works there now, at
  the cost of re-publishing this question's own 63 km/h figures.
...
## `Q89` — The low end needed the opposite correction, and tracking it was unstable

**Status.** ✅ Closed 2026-08-28 · **Owner.** `handling.tres` → `vehicle_controller.gd` ·
closes `Q88`'s open bottom · 🔴 **re-publishes the design-speed table**

**Claim.** `Q88` found the drift's usable band had a bottom nobody had tuned: below ~50 km/h it
returned 2.9–3.9° and the car *accelerated* through the manoeuvre. Fixed by
`drift_rear_grip_scale_at_low` (**0.44**) and `drift_low_fade_kph` (**41**), which deepen the cut as
speed falls — plus a **latch**, which is the part that matters.

| entry km/h | before | now | `secs>thr` |
|---|---|---|---|
| 34.51 | — | **35.1°** | 0.77 |
| 42.18 | 3.9° | **17.8°** | 0.42 |
| 49.48 | 2.9° | **49.2°** | 0.82 |
| 56.42 | — | **76.8°** | 0.80 |
| 63.02 | 51.1° | **69.8°** | 0.87 |
| 75.27 | — | **65.8°** | 0.95 |
| 86.36 | 50.4° | **50.4°** | 0.98 |
| 105.47 | 2.4° | **2.4°** | 0.00 |

✅ **The drift now works from 34 to 86 km/h** with no spin anywhere and dwell 0.42–0.98 throughout,
where the whole city-speed band was previously inert.

### 🔴 The yaw assist cannot substitute here, and that is measured

At 42 km/h, held drift peak slip against `drift_yaw_torque_nm`: **0 → 4.2°, 7000 → 3.9°, 14000 →
3.5°, 20000 → 3.6°**. More torque produces **less** slip, at the very top of the dial's range. With
grip unbroken the rotation is absorbed as a tighter line rather than a slide — `Q85`'s multiplicative
finding from the other side. So the low end is a grip problem and only a grip dial reaches it, the
exact mirror of `Q87`, where the grip cut was the culprit and no fade setting could reach it.

### 🔴 A tracking taper is UNSTABLE below the knee, and it broke the design speed

Built first as a continuous taper on current speed, symmetric with `Q88`'s. Measured:

| entry km/h | peak slip |
|---|---|
| 42.18 | 3.7° (still inert) |
| 49.48 | 94.6° |
| 56.42 | 164.1° |
| **63.02** | **165.0° — the design speed became a full spin** |

**A drift scrubs speed, so "deepen the cut as speed falls" is positive feedback**: slower → deeper
cut → more slide → more scrub → slower. Above the knee the same tracking is *stabilising*, because
losing speed moves the scale back toward `drift_rear_grip_scale`, the value everything is tuned at.
⚠️ **So the two halves are deliberately asymmetric and must not be "made consistent"**: the stable
branch tracks, the unstable one latches at engagement (`_drift_low_locked`).

✅ **Latching is what every constant-value sweep was already measuring**, which is why those sweeps
found a good drift at every speed while the taper did not — `Q88`'s method finding, now with a
mechanism. ⚠️ Latching *both* branches was tried and cost the high end: 86 km/h fell 50.4° → 20.6°,
because it stopped the high branch settling back onto the tuned base as the car slowed.

### 🔴 The design-speed table is re-published, for the first time in four changes

63.02 km/h moves **51.1° → 69.8°** (dwell 0.82 → 0.87), and the tap **16.0° → 20.5°** (0.23 → 0.40).
63 sits 2 km/h *below* `drift_fade_from_kph`, so it is inside the new taper by construction and no
tuning of these two dials avoids it — a taper that reaches the knee from below necessarily changes
everything just under it. **`Q84` and `Q86`'s 63 km/h figures describe a superseded car.** The three
previous changes each preserved them; this one cannot, and pretending otherwise would mean not
fixing the bottom at all.

⚠️ **The window is narrow and the two dials interact.** At 49 km/h `at_low` 0.44 gives 49.2° and 0.50
gives 5.2° — dead. At 56 km/h `low_fade` 41 gives 76.8° and 45 gives 104.7° — more sideways than
forwards. 41/0.44 is the balance: 42 km/h clears the threshold and 56 stays under 90°.

### Found by review, and by checking under the floor

🔴 **The `at_top` guard was left one call up from the path that reads it.** `_write_drift_grip` calls
`_high_grip_scale` directly, while the guard stood in the `_rear_grip_scale` wrapper — so a profile
leaving `drift_rear_grip_scale_at_top` unauthored would have lerped rear grip toward **zero** as
speed rose, which is the exact catastrophe that guard was written for. It was still in the file and
no longer on the live path. ⚠️ **This is the second time in three changes that a guard here was
walked past rather than removed** (`Q87`'s zero span was the first), and both times the diff read as
adding safety. The guard now lives inside the branch, and the wrapper — callerless, and duplicating
the knee test — is gone. Mutation-tested: `at_top = 0` returns **103.2°** at 86 km/h, exactly `Q87`'s
no-taper value, rather than a blowup.

🔴 **The latch released nothing, so a low-entry drift kept a 0.44 cut above the knee** — where that
depth spins the car. Gaining speed out of the low band is self-correcting for exactly the reason
tracking works above the knee, so the lock now **releases** at the knee. It deliberately never
**re-arms**: re-arming on the way down is the feedback loop, one step at a time. ⚠️ **Reasoned, not
measured — the skidpad cannot exercise it**, because a drift on the pad always decelerates, so no run
crosses back up through the knee. Reachable in game on a straight or a downgrade.

⚠️ **A re-press inside the 0.5 s release ramp does not re-latch**, since the gate is engagement-zero
rather than the button edge. A drift entered above the knee, released and re-pressed below it keeps
the base scale and is inert — the pre-`Q89` behaviour, so bounded and safe rather than dangerous.
Left as is: the alternative is re-arming mid-drift, which is the thing that must not happen.

🔴 **`_low_grip_scale`'s span guard was dead code, and that hid an inverted pair.** It sat below the
floor return, which already catches every speed the caller can pass whenever the pair is inverted —
so `drift_low_fade_kph >= drift_fade_from_kph` gave a flat `at_low` up to the knee and a step
discontinuity there, instead of the "behave as though the taper did not exist" every other guard in
this file promises. Ordered before the floor now.

✅ **Below `drift_low_fade_kph` the flat 0.44 is safe, which is what the floor is for.** 26.44 km/h
gives **35.1° / 0.55 s** and 17.97 km/h gives **13.4° / 0.00** with `decay/s` negative — mild and
accelerating, not a parking-speed spin. The band's real bottom is around 26 km/h.

**What is not settled here.**

- ⬜ **The tap is still dead below the design speed** — 3.9° at 42 km/h and 3.8° at 49, against 20.5°
  at 63. The held drift works there and the flick does not, and no dial here reaches it.
- ⬜ **0.44 and 41 are desk picks** off a narrow window; `P0-3b`'s handset decides whether 49 km/h at
  49° is fun or twitchy.
- ⚠️ **A 2 s hold at 50 km/h nearly stops the car** (52 → 3 km/h in the city). That is `Q50`'s
  isotropy and the player controls the duration, but it is a sharper cost at low speed than high.
- ⚠️ **None of this touches `Q85`'s four mechanism findings.**
...
### The sheet is to proportion, and that was checked

The page is stamped **NOT TO SCALE**, which would ordinarily make every proportion above worthless.
`RM1016` AUTOTOLL publishes its own `SIZE = 5600(H) x 2000` — a ratio of **2.800** — and its
pictogram measures **2.802**. The stamp means there is no scale bar, not that the symbols are
stylised. So the head and stem figures are evidence, and an overlay of the shipped glyph on TD's own
cell agrees on both.

⚠️ **`Q67`'s rasterise-and-diff still does not transfer.** That grades a sign face against a *scaled*
cell; this page carries no scale, so what an overlay can compare is shape at a normalised length —
which is what a proportion is, and is how the branch below was found to be wrong.

### 🔴 The branch was measured, shipped, and then authored anyway

**Reported from the driving seat the same day: "this arrow still doesn't make much sense."** The
measured branch — reach 0.150, head 0.100 long, barb span 0.233 — is **wider than it is deep**,
because what makes TD's head read as a direction is a pair of thin barbs sweeping back from the tip.
Drawn as this model's plain triangle that is a mushroom on the shaft.

Drawing it faithfully was tried and is worse. A six-point dart lifts the overlay agreement 0.831 →
**0.874** and renders as a **detached diamond**, because the concave rear pinches the head off the
arm. And TD's barbs are **0.09 m on a 4 m arrow** — below a lane stripe — so `Q91`'s sub-pixel
problem removes them at any driving distance: a faithful glyph that still reads as a blob from the
car.

So the branch ships as the one shape a straight arm and a triangle can state clearly — an arrowhead
**longer than it is wide**, 0.54 m by 0.46 m with its tip 0.76 m out, sized against the frame rather
than the sheet. `Q60`'s railing height and `P3-16`'s plate dimensions at a third layer, with the
difference that here something *is* published and is declined rather than missing.

⚠️ **The ahead head and the stem stay measured**, and this must not be read as licence to re-author
them: the overlay agrees on both, and they are the half of the glyph the sheet can be trusted for.
...
fewer lanes than the width brackets — an unpainted lane, expected, reported. **6** state more, and
...
**33** more agree with the count the width published: two readings sharing no input landing on one
integer, which is the only free cross-check either has. 🔴 **Filed against the PUBLISHED count and
not against the bracket, and the difference is 3 edges** — a `(1, 1)` bracket publishes
`LANES_FLOOR`, so a row of two on one sits *above* its bracket while agreeing exactly with what
shipped. Filed by bracket those read as the two instruments contradicting each other, and the
findings counter would be two populations wearing one name.
...
⚠️ **`Q94` was untouched by THIS change and that is not an oversight** (it was closed separately the same day — see `Q94`). `stacked_pairs` **89** and
...
the lane count, ✅ **since assigned by `Q94` on 2026-08-29** — the arrows' own row resolves the ambiguous brackets this width produces, which is what finally moved `stacked_disagreeing`; and `ground_clearance`'s two new edges, which belong to `Q24`.
