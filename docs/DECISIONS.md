...
| `Q79` | **The street plate needs a typeface, and the one that fits is not the one that was proposed** | ✅ Decided 2026-08-26 — **Free HK Kai** over Noto Sans HK on authenticity, because HK street plates are set in 楷書 and the Latin line staying in a grotesque is what the real plate does. 🔴 **AR PL UKai was disqualified on its licence, not its typeface** — the Arphic Public License bars commercial use. 🔴 **Hard rule 7's three owners become four**: a third-party font is not this project's code, not its authored asset and not government data. `tools/font_coverage.py` is the only thing standing between a data refresh and a tofu box |
| `Q80` | **A touch zone is not a thumb, and the first rule banned the corners every shipped game uses** | ✅ Closed 2026-08-26 — a tap zone is where input is **detected**, a thumb is what **occludes**, and conflating the two made the positions 3 of 4 shipped references use illegal. NFS No Limits is the disproof: it puts its speedometer squarely where the right thumb steers. 🔴 **The check now asserts the PERMISSION as well as the prohibition** — a rect over a tap zone must be *accepted* — so re-tightening onto zones fails the suite instead of silently re-banning the corners, since a stricter check otherwise looks like a better one |
| `Q81` | **A wrong-way sign is an interrupt, not a readout, and the nose decides rather than the wheels** | ✅ Closed 2026-08-26 — built on velocity and refused by the user, so the **heading** raises the sign and velocity may only **withhold** it: NO ENTRY's instruction is *turn around*, which a correctly-aligned driver must not be given. The stated cost is that a fast reverse up a one-way street is deliberately unsigned. 🔴 **An icon, not a word**, which deletes the ungraded-lettering branch. ⚠️ The region is **93.5% one-way**, so the false alarm is the failure mode and the **120°** bar is load-bearing |
