...
| `P3-11` / `P3-10` / `P3-7` / `P3-7a` / `P3-6` | Build `B2` — "it reads as HK" — **runs 1st** | 🟡 In progress | Taxi first (it is in every later screenshot), then ground (`Q18`), then the window shader, then the hero buildings. `P3-6` and `P3-7a` are the two not started. |
...
| `P3-7a` | └ Survey-driven façade variation | ⬜ Not started | Planned 2026-08-09 (`PLAN.md`): `W1` punched-as-glass (`Q44`) and `W2` per-building pane hue (`Q45`) first — both already judged wrong in `A″` — then `Q42`'s riders in reliability order: `emphasis` → reconciled storey pitch → `balconies` → podium. All dark behind `survey_apply`; each rider graded against a pre-fixed bar; **no new reader field before `Q26`** (a prompt change is a new graded run plus a paid re-survey). |
...
| `Q44` | **Punched openings draw as matte near-black holes** | 67% of read faces are `punched` — their windows read as holes, not glass, in the frame the driver faces on Hennessy Road | `P3-7a` | 🟡 Open — user's call 2026-08-09: punched windows are glass in aluminium frames and should behave as glass. Confirmed in the shader: recess-only colour, ~2% of the mirror head-on, wall roughness. ⚠️ Re-glassing re-spends part of `A″`'s `Q30` give-back — the bar is mean `C*` cost ≤ `A`'s on all three cameras |
