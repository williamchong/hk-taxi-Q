...
| `P3-7a` | └ Survey-driven façade variation | 🟡 In progress | `W1` punched-as-glass landed 2026-08-09: `unglazed_glassy` floors the opening's glassiness (0.0 = `Q43` exactly, 0.65 authored as the re-shoot's starting point); parked `C` frames byte-identical at `survey_apply = 0.0` on all three audit cameras. Next `W2` per-building pane hue (`Q45`), then the `A‴` re-shoot against the `Q30` bar, then `Q42`'s riders in reliability order: `emphasis` → reconciled storey pitch → `balconies` → podium. Each rider graded against a pre-fixed bar; **no new reader field before `Q26`** (a prompt change is a new graded run plus a paid re-survey). |
...
| `Q45` | **Four pane colours city-wide read as wallpaper** | Window tint repeats: three hashed tints plus one shared recess; the surveyed tint reaches ~226 of 2,214 buildings | `P3-7a` | 🟡 Open — user's call 2026-08-09: modulate the hashed fallback per building (seeded `L*`/`b*` jitter, a pull toward the measured hue). Never jitter the surveyed tint; punched panes never take the unconditioned dark-mode tint (`Q40`) |
