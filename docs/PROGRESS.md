...
| `Q47` | **A committed verdict is right about the tower, wrong about the ground band** | The tinted drive test put the residual wrong windows overwhelmingly on committed stock — `Q41`'s tower-biased verdicts paint the band the player drives past | `P3-7a` | 🟡 Open — **routes measured 2026-08-10, awaiting the route call**: `R4`'s podium bits are already 68.0% filled on committed stock (65.9% any-face / 50.9% majority on ≥40 m; 98% of values in 1–3 floors) with the floors→metres conversion written ahead of the call; the ground-band survey pass starts from zero paid reads; and the iB1000 third route is **verified to `DATA_SOURCES.md` grade** — 280 in-region podium blocks with base/roof levels 100% filled (p50 14.6 m), sub-metre footprint registration, 260 MB scriptable download. All three routes now stand on complete evidence |
