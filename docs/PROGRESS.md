...
doubled chroma is superseded** — it was compensation for a lightness problem `Q33` later fixed, and
the pair was never re-graded. Read the `Q36` row before raising the chroma again. The blue-illuminant
finding underneath it belongs to `Q31`'s owed rig pass.)

Previously: 2026-08-07 (**`Q34` closed: material is now a thing the city declares, not a thing the
schema implied from height.** A top-level `materials:` table holds all nine colours the city ships;
`buildings:` and `roads:` reference them by name, so `_check_exposure` is total because the *table*
is rather than because the loop is careful. `schema_version` 3, no ETL->game contract bump. Shipped
in two graded halves: a pure refactor verified **byte-identical across all 138 output files** bar
`city.json`'s timestamp, then the look change — a surveyed building draws its material from its
measured chroma and hue angle, seeded by a `blake2b` stream deliberately uncorrelated with the jitter
seed (⚠️ a prefix-salted `crc32` measures **+0.507** against it; CRC32 is affine over GF(2)). ⚠️ **The
fallback is the height ramp, not the surveyed marginal** — `facade_hue` is optional by contract, and
a survey-less clone was verified to build a byte-identical city before and after. Every bin's weights
are authored so its expected reflectance matches what the ramp gave that population, so the graded
result is a redistribution and not a level change: whole-frame `L*` **-0.8** street / **-0.1**
skyline, with ~32% of pixels moving a mean 2.2-2.5 `L*`. ⚠️ Two things it does **not** buy, both
recorded rather than discovered: it conditions **lightness on hue only** — `with_hue` discards the
drawn material's chroma a line later — and it buys **no spatial coherence**, since only 0.5% of hue
variance lies between sheets. `Q34`.)

Previously: 2026-08-07 (**the palette got a rule, and it overturned the audit's own headline
...
the albedo corrected against a material the remaining candidate is the shadow fill. ⚠️ **The rule's
own soft spot opened `Q34` the same day**: `reflectance` is a field on `HeightBand`, so the schema
asserts material is a function of height — measured at **0.9%** of `L*` and **1.4%** of `a*` for the
best geometric key of any kind, against **72.4%** of hue variance captured by five clusters on the
measured data. `Q33` `Q34`.)
...
| `P3-11` | └ Player taxi model | 🟡 **Awaiting review** | Generated by `tools/make_vehicle.py`; **1,132 triangles** in scene across two `.glb`s (556 body, 4 × 144 tyres). Untextured — the decal sheet is gone with the lettering it carried. Chassis taken from the scene, not chosen. Latest shots in `build/driver/v4_*`, earlier rounds in `build/driver/p311*`. |
| `P3-10` | └ Ground surface | 🟡 **Awaiting review** | Shipped 2026-08-05. Terrain is a tiled class: **+87,649 triangles** at LOD0, no texture, **no extra draw call**, **+4.56 MB of PCK**, and it **collides**. `ground_sink_m: 0.20` sized by `tools/ground_clearance.py`. Reads as ground on the flat core; **buries the carriageway on hill streets** — `Q24`. Review 2026-08-06 rejected the terrain-dominated view and `Q29` fixed what it found — the ground was smooth-shaded. Shots in `build/driver/q29*`, superseding `p310*`, which predate the palette. |
| `P3-7` | └ Window-band shader, and its `TEXCOORD_0` payload | 🟡 **Awaiting review** | One commit across both sides; `city.json` schema 4 → 5. Storey height **2.8 m measured**, not guessed. **+4.01 MB of PCK** (32.36 → 36.37), **zero triangles moved**, no extra draw call. Shots in `build/driver/p37*`. |
...
| `Q34` | **`reflectance` is a field on `HeightBand`, so the schema asserts that material is a function of height — and it is not.** Measured on the 2,171-building survey: height explains **0.9%** of `L*` once log pixel count is controlled (pixel count alone explains **22.0%**, and the two correlate at +0.420, so they are largely the same axis), and **0.7%** of `a*`. The best geometric key of *any* kind — height + footprint together — reaches **1.4%** of `a*`; slenderness `h/√A` is worse than either at 0.5%. Shadow-normalising to each building's brightest face does not rescue it (1.2%). Meanwhile **five clusters on measured hue capture 72.4%** of hue variance, so the structure is real, strong, and simply not a function of shape | `Q33` made reflectance *citable evidence*, but a claim is only evidence where it attaches to the thing it is about. On a height bucket, "48.7% = grey painted render" reads as **"buildings under 12 m are grey painted render"** — a claim about one city's stock that the data refuses, and exactly why `hong_kong.yaml` still has to warn that the five band reflectances are "back-derived, not cited". It is unfixable in place: no source can support the sentence. It also breaks **hard rule 3** — a `materials:` table is portable, because concrete is concrete in the second city, where a height→material mapping must be re-derived from scratch | `config.py` | 🟢 **Closed 2026-08-07.** Shipped as `Material(name, colour, reflectance, source)` in a top-level `materials:` table, plus `material_assignment` with a hue-conditioned draw over chroma rings and hue sectors. `schema_version` 3; no ETL→game contract bump, because no output artefact records a reflectance. ⚠️ **Two claims made while designing this were wrong and are corrected below rather than deleted**: the fallback is the height ramp and *not* the surveyed marginal distribution (a marginal draw breaks `facade_hue`'s optional-by-contract guarantee — a survey-less clone must build the same city, and that was verified byte-for-byte); and the draw conditions **lightness on hue only**, because `with_hue` replaces `a*`/`b*` a line later, so the drawn material's own chroma never reaches the screen. What it buys is that a cream-rendering building gets an albedo plausible for cream (60-75%) rather than for concrete (20-30%). Stage A was verified byte-identical across all 138 outputs; Stage B graded at whole-frame `L*` -0.8 street / -0.1 skyline with ~32% of pixels moving a mean 2.2-2.5 `L*`, because every bin's weights are authored to match the reflectance the ramp already gave that population. ⚠️ **Spatial coherence remains open and is the live half of this** — see the row below. Original proposal, kept because the reasoning is what selected the shape: `Material(name, colour, reflectance)` as a first-class table plus a **stated assignment rule**, with height demoted to one option among several and a weak one. ⚠️ **The bands do three jobs and only one is wrong** — the lightness ramp (a *visual* rule, legitimately keyed to height), the material claim (must move), and per-building variation (`colour_jitter` and `facade_hue` already do it correctly). Assignment candidates: a **seeded weighted draw** on the existing `_seed(mesh)` — `crc32` of the LandsD object id, already stable across runs and already shared by the jitter and `P3-7`'s window phase, so it costs no new data, no payload and no schema on the game side; a **hue-conditioned draw**, which is the two combined and reads as the strongest option — `P(material \| hue bin)` authored in the config with the seed drawing inside it, falling back to **the height table** where the survey has no row. ⚠️ **Not to the marginal distribution** — that was the first proposal here and it is wrong: `facade_hue` is optional *by contract* (`buildings.py`), so a clone without the 4.9 GB survey must build the **same** city, and a marginal draw would build a different one. Recorded so it is not re-proposed. Its argument is *consistency* rather than accuracy: `facade_hue` already puts a measured hue on the building, so a material assigned independently of it can contradict the colour already on screen. ⚠️ **The honest claim is narrower than "removes the contradiction by construction"**, which is how this was first written: `with_hue` replaces `a*`/`b*` and keeps only `L*`, so for a *surveyed* building the drawn material's own chroma never reaches the screen. What the draw conditions is **lightness on hue** — a cream-rendering building gets an albedo plausible for cream (tile 60–75%) instead of one plausible for concrete (20–30%). Measured bins over 2,171 buildings — cool/blue-grey **34.1%** (`a*` -0.9, `b*` -2.5), neutral grey **36.3%**, warm grey **20.1%**, cream/beige **5.9%**, strong ochre **3.6%** (`b*` 25.8) — so ~70% is near-neutral and the distinctly warm tail is ~10%. ⚠️ **Bin by authored chroma/hue-angle thresholds, not by k-means**: cluster centres are data-dependent, so they shift per region and per city and break hard rule 3, where thresholds are portable and deterministic. ⚠️ **It does not buy spatial coherence** — hue is unstructured at the only resolution testable here, with **0.5%** of hue variance lying between the six sheets; block-scale coherence is untested, because the survey carries `sheet` but no coordinates. Or an external zoning/age join, unscouted and buying accuracy the player cannot see. Open sub-decisions: which rule ships; whether `height_bands` survives at all as a pure lightness gradient, given its span is **6.0 `L*`** against jitter's **±3.4 `L*`**; and **spatial coherence** — a per-building draw across materials with wide reflectance gaps gives a salt-and-pepper skyline where real blocks share cladding, which wants deciding before it is built, not after it looks wrong. ✅ **No colour has to move on day one**, so this can ship as a pure refactor and be graded apart from any look change; `_check_exposure` is already whole-config and keeps working unchanged. ⚠️ **Not a route to fixing the numbers by measurement** — the photographs cannot supply albedo (`Q33`) and cannot see material either: texel ground sample distance is **13–18 cm**, so mosaic tile joints (3 cm), brick courses (7 cm) and render texture are all below resolution, and only the window/mullion grid at 2.4 m resolves. The one cut the imagery could support, glazed vs solid, is also the one that must not be used raw: curtain-wall glass has a **diffuse** reflectance of 8–15%, which flat-shaded renders a tower near-black, because real glass towers are bright by specular sky reflection that this renderer does not do |
...
`Q18`'s first half came back **flat does not read**, given on `build/driver/q18oblique` — a
waterfront view that is mostly terrain. But what reads as plaster dunes there is *shading*, not
colour, and the question had assumed colour.

⚠️ **The stale-shot trap came first, and it is the reason this went unanswered for a day.** `Q18`
pointed at `build/driver/p310*`, shot 2026-08-05. The palette commit later that day moved
`TERRAIN(TB)` from `#b0a99a` to `#837d72`, so those shots showed a ground colour that no longer
existed. The tiles were already current — every one of the 133 postdates `hong_kong.yaml` — so
nothing needed rebuilding, only re-shooting. **A verdict pending on a screenshot has an expiry date
that nothing in the repo records.**

**LandsD ships the terrain faceted.** All three vertex normals agree on every one of the sheet's
83,637 source triangles. `mesh.collapse` is what smooths it: the `height_field=True` path drops the
facing term from the cluster key — correctly, since keeping it tore the sheet open, 99.61% region
coverage against 99.84% — and `_cluster_mean` then averages the normals of whatever shares a 4 m
cell. Measured on the real sheet: **mean normal error 8.72°, and 27.5% of vertices off by more than
10°.** Every other surface in the city is faceted, so the ground read as plaster.

**Fixed in the fragment stage, gated on `MARKER_GROUND`, in both facade shaders.** The normal is
rebuilt from the derivatives of view-space position, which is exact for a flat triangle:
**zero triangles, zero vertices, zero draw calls, and no geometry change at all** — the diff is two
shaders. The ETL alternative, unwelding the ground for per-face normals, costs ~219k more vertices,
about **8 MB against a 32.3 MB PCK**, a quarter of the bundle for one class's lighting.

Two details that are load-bearing rather than stylistic. The derivative is taken **unconditionally**,
not inside the `marker` branch: derivatives are undefined where a pixel quad straddles a non-uniform
branch, and a quad on a seam straddles two triangles whose marker differs. And the result is
**oriented against the interpolated normal** rather than by a hand-picked sign, because the cross
product's direction follows winding and projection handedness — `sign()` is avoided in favour of a
comparison, since it returns zero on a perpendicular pair and would hand the mix a zero vector.

Graded at three viewpoints. Oblique **6.5% of pixels changed with frame `L*` 65.93 → 66.21**, so it
is structure and not exposure; the terrain expanse goes from smooth gradient to discrete steps,
neighbour-step over 1 `L*` on **2.74% → 3.28%** of pixel pairs. The two fixed viewpoints move 3.4%
and 4.6% — ground pixels only, frame `L*` within 0.11 — because facades take the mix at weight zero.
`check.sh` exits 0 with **0 `SHADER ERROR`**, and `export.sh web` builds clean, which is the check
that mattered for a derivative.

⚠️ **The land-cover classifier `Q18` defers would not have fixed this, and that was measured rather
than assumed.** `ART_DESIGN.md` specifies putting the class in `collapse`'s cluster key, which
genuinely does change which normals average — **47.8% of cells hold more than one class**, so it
splits often. It splits by *colour* where the error comes from *slope*, and on this terrain the two
are uncorrelated: adding the class to the key moves the normal error **8.72° → 8.15°, 6.5%**, while
raising the cluster count **8,718 → 14,770 (+69%)**. A 19th of the fix, paid for in geometry.

**So `Q18`'s colour half is not earned yet and is deliberately left open.** Both the "does flat read"
verdict and the wash numbers behind it were taken on a smooth surface; a faceted one responds to
light differently. Re-judge colour on the fixed ground. The measured case against photographic
terrain colour also stands on its own and is recorded with the `Q18` evaluation: the source JPEG's
variance is overwhelmingly `L*`, and that `L*` is **shadow** — off 151 building footprints the
sunlit-minus-shadowed contrast grows monotonically with the occluder, −0.88 `L*` under 15 m against
+10.70 `L*` above 60 m — which would bake the flight's sun into a ground the engine shadows again,
in a different direction, under two lighting scenes.
...
Asked for directly, as a list: no lettering anywhere, no black bars on the flanks, no bezel round
the tail lamps, no silver blocks in the grille, and **the bumper made part of the body rather than a
bar stuck to it**. All of it granted. **1,132 triangles** in scene — 556 body, 4 × 144 tyres —
against `ART_DESIGN.md`'s 2,000. Shots in `build/driver/v4_*`.

**The bumper is the change worth recording.** It was two boxes standing 6 cm proud of each end, and
at the size the car is actually drawn the front one was the widest thing on it. It is now a *colour
band*: `lower_profile` carries a fifth ring at `bumper_top_y_m` whose inset and cut are interpolated
from its neighbours, so the ring changes nothing but where `_loft` changes colour, and the
silhouette is identical with it and without it. **It stops at the corner chamfers, and that is the
final answer, not an artefact of `_loft` skipping the flank edges** — the band was continued down
the sides for one round and came straight back as *"side bottom should also be red without a black
bar"*. The flank is now red from sill to belt and carries nothing at all: no rub strip, no sill
skirt, no band.

⚠️ **Three rounds, three corrections, all of them the same shape.** "Make the bumper part of the
body" was first built as *delete the bumper*; "remove the characters from the decal" as *delete the
decal*; the bumper band as *a band all the way round*. Each time the instruction described where
something should **end up**, and it was read as permission to remove it. Worth holding onto: when a
note says a feature is in the wrong form, the default is to change its form, and deleting it needs
its own reason.

⚠️ **Removing a part is not free when other parts are mounted on it.** The plate and the fog lamps
were bolted to the bumper bar, which was flat; the nose it fell back onto is not — the body draws in
by up to 14 cm between the belt line and the sill. `_seated_depth` answers that by deriving each
fixture's depth instead of taking a thickness: back to where the panel is furthest in, face
`FIXTURE_PROUD_M` clear of where it is furthest out. It then produced a *correct* answer that looked
wrong — the badge spanned the sill tuck, so bridging the fold made it stand 4 cm off the paint at
its bottom edge and hang below the car's own outline. The fix is placement, not depth, and a test
now holds every fixture above the fold.

⚠️ **`_seated_depth` sampled two points of a profile that has three.** Found in review, after the
shots above were taken. `face_inset_m` is **not monotonic** — the body is furthest out at
`belt_y_m - bevel_m * 1.4`, in the *middle* of the range, not at either end — so bracketing it with
the band's two edges misses the extreme. The top tail-lamp lens is the one fixture that straddles
that knot, and it was standing **8.5 mm proud where 15 mm was promised**. Nothing showed, because
the shortfall was smaller than the margin; shrink `FIXTURE_PROUD_M` and the lens sinks into the
paint at mid-height only. It samples every knot inside the band now, and the test asserts the full
`FIXTURE_PROUD_M` at each of them rather than merely "greater than zero" — the weaker assertion is
what let this pass. **A contract test that checks the sign of a quantity does not check the
quantity.**

⚠️ **`_seated_depth` compensates in y, and the corner chamfer is a function of x** — recorded as a
known limit rather than fixed. A fixture outboard of `half_width_m - corner_cut_m` overhangs the
chamfer and its back cap floats clear of the paint: **10 cm on the tail lamps, 9 cm on the
indicators**. Pre-existing in kind and slightly better than it was — the bezel this replaced
overhung further — and invisible at the angles the car is actually drawn at. The docstring used to
promise more than it delivers; it now states the limit.

**Text is gone, and with it the third material.** The roof sign, both plates and the 4 SEATS badge
carried pixels from a 256² sheet; a bitmap font was the one surface on the car that nothing else in
this city has. `tools/vehicle_decals.py` and `taxi_decal.glb` are deleted, and **the car is
untextured all the way through** — which closes the exception `P3-11b` took against
`ART_DESIGN.md`'s 1–2 materials rather than merely leaving it unused. What survives is the part that
was never text: the plates are flat colour, white at the front and yellow at the rear per the HK
standard, and that asymmetry is still what proves the model is not mirrored. **The badge came back
after a first pass deleted it** — the user's correction was that a green dome without lettering is
still the badge — so it is geometry now, a faceted half-disc, and `BADGE_GREEN` takes the palette to
**seven** where `ART_DESIGN.md` says 3–5. Flagged on the same terms as `AMBER` before it.

⚠️ **The tail cluster's bottom lens is now invisible, and it was predicted.** `P3-11b` recorded that
the dark bezel existed *because* a red lens on red bodywork vanishes. The bezel was removed as asked
and the lens does vanish: the cluster reads as amber over white, with a bump where the red should
be. The earlier wrong fix — making it cream — is a white tail lamp, so it is not available. Live
options are a bezel again, a darker red outside the palette, or raising `bumper_top_y_m` until the
band reaches the lens.

**`SILVER` came down from (226, 228, 230) to (168, 172, 178)** — the roof read as *white* rather
than silver, reported from a screenshot. The swatch was never the problem: at the exposure the city
is lit at, an albedo that high leaves no headroom, so the roof has to be chosen against a render and
not against the colour. It is the cant rail, the wheel hubs and the door handles too, all of which
wanted the same correction. Measured on the same frame either side: roof pixel **145 → 111**.
...
pays with the elements on, where `solid_share` of every block has no grid on it at all.
⚠️ **It is derived, so it can rot: anything added inside that block which draws while all five
predicates are off has to be added to the predicate.** The shader header says so at the guard.
Verified behaviour-preserving by rendering both `Q27` viewpoints before and after: **707 and 804
pixels of 2,073,600 differ, by at most 1/255**,
...
**The city has window rows.** `assets/shaders/city_facade.gdshader` bands every vertical facade in
world space — no texture, no atlas, no second draw call — reading two things the ETL now ships in
`TEXCOORD_0` because a vertex cannot know them. `ARCHITECTURE.md` holds the contract; what belongs
here is what was decided, what was measured, and the two things that were nearly wrong.

| | before | after |
|---|---|---|
| LOD0 / LOD1 triangles | 521,693 / 253,097 | **unchanged** |
| Draw calls per tile | 1 | **1** |
| **PCK** | **32.36 MB** | **36.37 MB** (+4.01) |
| ETL peak RSS | 800 MB | 900 MB |
| `city.json` `schema_version` | 4 | **5** |

**The storey height was measured off the real facades rather than chosen, and the guess would have
been wrong.** `ART_DESIGN.md` sanctions a one-off offline read of the *individualised* (textured)
LandsD set for colour; this extends it to spacing. One sheet, 1.10 GB, fetched by changing a single
character in the download URL (`GLTF0` → `GLTF`), autocorrelated down each wall texture's V axis, and
discarded:

| | value |
|---|---|
| usable walls / buildings | 227 / 219 |
| floor pitch p25 / p50 / p75 | 2.25 / **2.70** / 3.54 m |
| height-weighted median | **2.77 m** |
| column pitch p50 (88 walls) | **2.42 m** |
| row contrast p50 / p90 | 0.126 / 0.309 |

Shipped as **2.8 m**. The obvious guess was 3.2 — a Western commercial storey — and at 40 storeys
that is five floors of error on one tower. Hong Kong's domestic floor-to-floor really is that tight.

⚠️ **The eye said the contrast was too strong and the measurement said the opposite.** The first
render read as harsh, and `window_opacity` was dropped 0.62 → 0.45 → 0.30 on that judgement. Then the
*rendered* frame was measured the same way the photographs were — p10-p90 of row-mean luminance over
one facade patch — and came back **0.107 at 0.62**, against **0.126** in the source. It was already
under. It went back to 0.62. A dark square on a pale wall in full sun reads far stronger than its
share of the row, and `ART_DESIGN.md` also records that oblique aerial capture is haze-flattened, so
0.126 is a floor rather than a target.

⚠️ **The probe's first answer was a measurement of its own window, not of the city.** It reported a
median floor pitch of exactly **2.00 m** — the lower bound of the search range. Autocorrelation decays
monotonically away from lag 0 unless something genuinely repeats, so `argmax` over a window returns
the window's own left edge on every aperiodic wall, and a facade's street-level-to-sky brightness ramp
is a far stronger signal than its floor lines. Detrending first and taking a *local peak* rather than
the maximum moved the median to 2.70 m. **A statistic that lands exactly on a limit you chose is
reporting the limit.**

⚠️ **`mesh.merge` refused the whole task, and nothing said so until it was tried.** It rejected any
mesh carrying UVs at all — one condition covering two questions — so every tile, which is merged twice
(per class, then per tier), would have failed. The texture half is untouched and still right: two
textures cannot share a primitive without an atlas. UVs with no texture are a shader coordinate and
merge like any other attribute. Nothing in `PLAN.md` or `ARCHITECTURE.md` had noticed the guard.

⚠️ **A float32 rounding trap, found while writing the packing and not by any test.** The marker and
the phase share one float as `marker + seed`. The raw seed reaches 1 − 2⁻³², and float32's spacing
near 2.0 is ~2.4e-7 — so `STRUCTURE + 0.9999999998` rounds to **exactly 3.0**: an unknown marker with
a zero phase, on whichever viaduct happened to draw a high seed. Rare, silent, and geometry-dependent.
The phase is quantised to 1/256, which is exactly representable at every marker, so `floor` and
`fract` round-trip instead of nearly doing so. `test_a_high_phase_never_rounds_into_the_next_marker`
brute-forces all 768 combinations rather than sampling, because the failure was one value at one end
of one class.

**Three places must agree, and only one of them can fail loudly.** The ETL names the glTF material
...
**which is exactly what the city looked like before this task**. No error, no missing file, nothing on
screen that reads as broken. That is why `verify_tiles.gd` now asserts both the `TEXCOORD_0` format
and the resolved material path: it was checked by hand once, and a check performed by hand is a check
that will not be performed again.

**The marker is derived from the palette rather than from a new config key.** A class with a flat
`class_colours` entry is one whose colour does not depend on its height, which is exactly the set with
no floors to band; anything the height bands colour is a facade. Hard rule 3 holds — no class name
reaches pipeline logic — and a second city gets the right answer from its own palette. `FACADE` is the
fallback rather than a listed case, so a new massing class bands until someone gives it a flat colour.

**Two things the payload deliberately does not carry.** The horizontal window coordinate is derivable
in the shader from world position and the wall normal, and shipping it would cost bytes on every
vertex of every tile forever. And the grounding gradient `ART_DESIGN.md` wanted baked into `COLOR_0`
is now a `smoothstep` on `UV.x` — baking it would have forced `colour_for` to materialise a per-vertex
array where it returns a four-byte broadcast view.

⚠️ **The distance fade covers the floor lines too, which the first cut got wrong.** Only the glazing
was faded, so a hairline grid survived to the horizon — off-spec against the 250-400 m tier's
"silhouette-only, flat colour", and the worst part to leave in, being one pixel wide by definition and
therefore the part that aliases.

⚠️ **`ARCHITECTURE.md` predicted "~2 bytes/vertex quantised" and this ships float32 at four times
that.** Measured rather than argued: +4.01 MB of PCK over 937,889 vertices, 7.50 MB of raw VEC2 that
the pack compresses by 47%. `unorm16` would save perhaps 2 MB and costs a scale factor in the contract
on both sides; not done, because the bundle budget is 200 MB and this build is at 36.37. Recorded so a
later region that is short of room knows where the 2 MB is.

**A stale figure corrected in passing:** `ARCHITECTURE.md` still described `bounds_game` as
1728 × 977 m from `P3-10`. `Q25` moved the ground and it has been **1737 × 977 m** since; the baseline
build at `HEAD` confirms it, so it is not this task's doing.
