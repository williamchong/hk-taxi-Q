...
| `Q41` | **A vision reader recovers the grammar the statistic could not** | A validated reader survey dissolves `Q26`'s "invented surface" objection to candidate `A` | `P3-9a` | 🟡 Open — pilot read every well-photographed face correctly, incl. both of `Q40`'s misclassifications; `tools/facade_unwrap.py` committed (depth buffer kills mode-5 foreign geometry); 40-face validation set labelled and thresholds recorded **before** the reader's first run. Blocked on: the graded validation run, then a human spot-check of the labels |
