...
| `Q47` | **A committed verdict is right about the tower, wrong about the ground band** | The tinted drive test put the residual wrong windows overwhelmingly on committed stock — `Q41`'s tower-biased verdicts paint the band the player drives past | `P3-7a` | 🟡 **Route decided 2026-08-10** — the sequence: iB1000 `P`-block metres where a committed tower meets one (54.8% of towers; precedence **data > survey-inferred**, extending the `authored > survey > hash` ladder), `R4`'s floors→metres conversion on the complement with the 668-tower iB1000 overlap as its validation set (`Q42`'s bar discharged by a join, not hand labels), and the ground-band batch unchanged in its shopfront-only scope. Closes when the shipped boundary is graded; iB1000 ingestion ✅ **landed 2026-08-10** (`P3-7a` — decoder, fetch, verified read reproducing the record's counts), the next queued task is the tower↔block join with mechanism-won provenance |
