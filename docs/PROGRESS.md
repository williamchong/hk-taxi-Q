...
| `Q40` | **Can façade grammar be surveyed instead of hashed?** | `Q26`'s objection to `A` is that it abandons the accurate surface — a surveyed grammar would have dissolved it | `P3-9a` | 🟡 Open, **narrowed**. 🔥 **Grammar killed by the validation sheet — 1 right, 8 wrong, 3 unsure over 12 graded faces.** `L*` modulation measures materials, not recession, so a dark-glass curtain wall reads "punched" at `L*` 17.4; glass reflections are most of the signal; one lattice screen reads `fin` from W and `curtain` from S. `Q26`'s objection to `A` therefore **stands**. ✅ The **world-space unwrap works** and atlas-space is dead (5,831 charts, only **15.2%** analysable). ✅ Glazed-vs-blank and a 2-D `L*`×`b*` tint survive — they rest on a bimodality dip, not periodicity — behind a **resolution gate**. ✅ **The contamination check is discharged** (`tools/facade_glazing.py`, committed): one dip statistic from both texel selections moves the verdict on **19 of 51** gated buildings on `9D` and **195 of 699** on `14B`, in both directions — Probe 1's numbers describe an uncommitted statistic over a contaminated selection. ✅ **The survey extension is built and ran region-wide** (2,171 buildings, six sheets: dip + dark/light `L*`×`b*` tint tables). 🔥 **The re-derivation killed the dip gate**: calibrated against `Q41`'s reader (1,629 gated buildings), best Youden **0.100** at any threshold, conditional medians flip sign across sheets, and blank-vs-windowed runs *inverted* (blank median dip 0.484 vs windowed 0.850) — an `L*` split cannot tell glass from shadow, so **glazing is the reader's call** and a refusal falls to the hash. ✅ **Tint survives conditional on the reader**: dark-mode `b*` orders blue −4.83 < green −2.62 < neutral −1.92 against the reader's independent enum; dark-bluer-than-light 89% region-wide (was 77%). 🔥 **The warm-glass gap dissolves**: 17/56 → 7/51 decontaminated → **0/15** conditioned on reader-glazed on the same sheet; region-wide 6%, `b*` p90 +1.27 — the authored cool palette stands, no config moves, `facade_chroma`/`ring_weights` not triggered. Remaining: `TEXCOORD_1` plumbing only. ⚠️ `Q41` narrows the grammar kill to *per-pixel statistics* — the data carries the signal |
| `Q41` | **A vision reader recovers the grammar the statistic could not** | A validated reader survey dissolves `Q26`'s "invented surface" objection to candidate `A` | `P3-9a` | 🟡 Open — ✅ **the reader PASSED its graded run, first run, no label corrected**: strict grammar **19/20** (bar 16), marginal **6/6** (bar 5), refusal **14/14** (bar 13), glazed **23/24** (bar 90%), on `claude-opus-5` prompt `61fae4b11722bb5c`, against thresholds fixed *before* it ran. `Q40`'s grammar kill is therefore confirmed **narrow** — unreachable by a per-pixel statistic, reachable by a reader — and `Q26`'s "invented surface" objection to `A` is the thing that moves. ✅ Refusal is decisive: all 14 refusal-pool faces refused at *high* confidence, most marginal calls at *low*. ✅ Replay verified — a rerun made **zero** API calls for an identical verdict, needing neither SDK nor credential. The single strict miss is `blank`-versus-refusal, the taxonomy's own boundary, ⚠️ **unadjudicated rather than upheld** (the re-inspection clause was never exercised). ✅ **The cache is now keyed by the image**: every entry's key and every output row carry the encoded PNG's `blake2b` fingerprint, so an unwrap change makes a stale entry unfindable rather than silently replayed; the 40 paid entries were backfilled by pure rename and the no-credential replay re-verified — zero API calls, identical verdict. The row cache is keyed by `UNWRAP_HASH` so a row never outlives the code its images came from. ✅ **The human label spot-check passed, no corrections** (2026-08-09, side-by-side against native unwraps, reader answers withheld). ✅ **Resurveyed onto `claude-sonnet-5`** (2026-08-09, prompt `d6e59b39307b215d`) by the pin's own rule: graded 18/20, 6/6, 14/14, **24/24** at two-fifths of Opus's price, after the full-region run was costed at 8,661 calls / ~$103 on Opus; Haiku 4.5 failed the same gate (strict 15/20, refusal 12/14). ✅ **The full-region survey is complete** (2026-08-09, ~$21 on the Batch API — transport only: same request bytes, results land in the response cache, all six tables authored by a credential-less replay, zero API calls): 8,614 paid reads, zero transport errors; **54% of faces read, 80% of buildings ≥ 1 read face**; `punched` 67% of reads, and `curtain` leads only on `11-SW-9D`'s tower cluster. Remaining: the `TEXCOORD_1` channel design with `Q40`, and `Q42` per-field validation before consumption |
| `Q42` | **The reader answers seven questions nobody consumes** | `storey_count`, podium, balconies, emphasis and signage are collected on every paid call the full-sheet run will make anyway — free survey data if validated, a second `schema_version` bump avoided if packed into `TEXCOORD_1` now | `P3-9a` | 🟡 Open — from the 40 paid validation responses: storey pitch median **3.32 m/floor** against `Q40`'s independent **3.38 m** (two instruments, shared photography only), `emphasis` 25/25 and coherent with grammar, `band_period_floors` 0/25 (never commits). 🔴 `signage` reads real trademarks (SHUI ON GROUP, REVENUE TOWER, YMCA, FWD) — identification and placement only, never rendered text. Nothing consumed yet; each field owes its own validation before plumbing |
