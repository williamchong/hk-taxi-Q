...
| `P3-7a` | └ Survey-driven façade variation | 🟡 In progress | `W1` punched-as-glass and `W2` per-building pane colour landed 2026-08-09: `unglazed_glassy` floors the opening's glassiness (0.0 = `Q43` exactly), and the hashed pane pick takes seeded `L*`/`b*` jitter plus a `COLOR_0` hue pull, fallback only, in the vertex stage. Parked `C` frames byte-identical at `survey_apply = 0.0` on all three audit cameras after each step. Next the `A‴` re-shoot against the `Q30` bar and the user's verdict, then `Q42`'s riders in reliability order: `emphasis` → reconciled storey pitch → `balconies` → podium. Each rider graded against a pre-fixed bar; **no new reader field before `Q26`** (a prompt change is a new graded run plus a paid re-survey). |
