...
| `Q40` | **Can façade grammar be surveyed instead of hashed?** | `Q26`'s objection to `A` is that it abandons the accurate surface — a surveyed grammar dissolves it | `P3-9a` | 🟡 Open, **feasible**. No published attribute; the evidence is the individualised photography. ✅ The **world-space unwrap works** — 1,837 UV islands stitch into a legible elevation. ⚠️ Atlas-space analysis is dead: 5,831 charts, median 3.8 m², only **15.2%** of wall area spans 3 bays and 3 floors. Glazed-vs-blank and a 2-D tint are measurable behind a **resolution gate** — at low resolution "no windows" and "badly photographed" read identically. ⚠️ **17 of 56 buildings measure a warm glass the three authored tints cannot express.** 🔴 The grammar classifier has no "I don't know" and turns two measurement failures into confident calls — validation sheet owed before any contract change |
