...
**Three things are knowingly missing from the world.** There is **no ground** — between the roads
and under the buildings is skybox (`P3-10`, build `B2`). The **elevated network is closed to
driving** — `nearest_edge` refuses all 60 off-grade edges (`Q13`, reopened deliberately in Phase 4).
And **nothing in the authenticity table is built** — no traffic, no trams, no neon (`P3-3`, `P3-4`,
`P3-8`).
...
| `P3-11` | └ Player taxi model | 🟡 **Awaiting review** | Generated by `tools/make_vehicle.py`; **772 triangles** in scene across two `.glb`s. Chassis taken from the scene, not chosen. Review shots in `build/driver/p311*`. |
...
| `Q18` | Does flat-coloured ground read as ground, or does it need land-cover colour classified from the source aerial JPEG? And does sinking terrain ~0.2 m under the road deck clear the carriageway on cross-slopes? | Decides whether `P3-10` stops after its cheap half or grows an image-decode stage and a **Pillow** dependency. Get the z-fighting half wrong and the ground fights every road in the region | `P3-10` | 🟡 Open — see the note below |
