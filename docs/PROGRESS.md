...
| `Q40` | **Can façade grammar be surveyed instead of hashed?** | `Q26`'s objection to `A` is that it abandons the accurate surface — a surveyed grammar would have dissolved it | `P3-9a` | 🟡 Open, **narrowed**. 🔥 **Grammar killed by the validation sheet — 1 right, 8 wrong, 3 unsure over 12 graded faces.** `L*` modulation measures materials, not recession, so a dark-glass curtain wall reads "punched" at `L*` 17.4; glass reflections are most of the signal; one lattice screen reads `fin` from W and `curtain` from S. `Q26`'s objection to `A` therefore **stands**. ✅ The **world-space unwrap works** and atlas-space is dead (5,831 charts, only **15.2%** analysable). ✅ Glazed-vs-blank and a 2-D `L*`×`b*` tint survive — they rest on a bimodality dip, not periodicity — behind a **resolution gate**. ⚠️ **17 of 56 buildings measure a warm glass the three authored tints cannot express.** 🔴 Blocked on one check: the unwrap admitted trees and a neighbouring building, and the same wall selection feeds the glazing histograms |
