...
| `Q47` | **A committed verdict is right about the tower, wrong about the ground band** | The tinted drive test put the residual wrong windows overwhelmingly on committed stock — `Q41`'s tower-biased verdicts paint the band the player drives past | `P3-7a` | 🟡 Open — `W4` is disqualified at scale; candidate routes are `R4`'s podium bits (collected by the reader, bits reserved unpacked; conversion precondition stands) or a ground-band survey pass, and the fill-rate rider order now has an impact counter-argument to record either way |
