...
| `P3-7a` | └ Survey-driven façade variation | 🟡 **Awaiting review** (`W1`/`W2`) | `W1` punched-as-glass and `W2` per-building pane colour landed 2026-08-09; parked `C` frames byte-identical after each step. **`A‴` shot** (`build/driver/q26_A3_422ee16/`): responding share unchanged from `A″`, p90 \|d`L*`\| down a third at `street`/`kerb`, `Q30` bar held (+1.16/+0.78/−0.05 against `A`'s +2.28/+1.43/−0.02). ⛔ **Gate: the user's verdict** — *do punched windows read as windows, and do two adjacent towers still read as two buildings?* — before any `Q42` rider. Then riders in reliability order: `emphasis` → reconciled storey pitch → `balconies` → podium, each against a pre-fixed bar; **no new reader field before `Q26`**. |
...
| `Q44` | **Punched openings draw as matte near-black holes** | 67% of read faces are `punched` — their windows read as holes, not glass, in the frame the driver faces on Hennessy Road | `P3-7a` | 🟡 Open — mechanism landed 2026-08-09 (`P3-7a` W1): `unglazed_glassy` floors the opening's glassiness, spanning both `Q44` candidates in one tunable (0.0 = `Q43` exactly, 1.0 = opening material always glass); 0.65 authored as the `A‴` starting point; parked frames byte-identical. ✅ The `Q30` bar held on the `A‴` re-shoot: +1.16/+0.78/−0.05 against `A`'s +2.28/+1.43/−0.02, only `kerb` re-spending. ⚠️ Still owed: the user's verdict on the frames |
| `Q45` | **Four pane colours city-wide read as wallpaper** | Window tint repeats: three hashed tints plus one shared recess; the surveyed tint reaches ~226 of 2,214 buildings | `P3-7a` | 🟡 Open — mechanism landed 2026-08-09 (`P3-7a` W2): seeded `L*`/`b*` jitter plus a `COLOR_0` hue pull on the hashed fallback, hoisted to the vertex stage; the surveyed tint is untouched and punched panes cannot take the dark-mode tint (ETL-enforced). 6.0/4.0/0.25 authored as `A‴` starting points. ✅ On the `A‴` re-shoot the modulation costs chroma nothing — `street` cost *fell* vs `A″`. ⚠️ Still owed: the user's verdict; the bound is `Q35`'s salt-and-pepper skyline |

**Closed:** `Q1` `Q2` `Q3` `Q4` `Q5` `Q7` `Q8` `Q9` `Q10` `Q11` `Q12` `Q16` `Q17` `Q18` `Q20` `Q23`
`Q25` `Q27` `Q28` `Q29` `Q32` `Q33` `Q34` `Q34′` `Q36` `Q37` `Q40` `Q41` `Q43`. Each has a record in `DECISIONS.md`.
