...
| `Q37` | **10.0% of the shipped façade survey is atlas filler, not a photograph** | 222 of 2,214 buildings render dead-neutral because they were not measured, and `Q34` then assigns them a material from absent data — but on `11-SW-9D` the filler reaches **20 of 59**, so the achromatic rows are one in seven of the buildings affected | `buildings.py`, `tools/facade_survey.py` | 🔴 Open, and **blocked on a decision, not on work**. The replacement survey is written, tested and validated on `11-SW-9D`: zero `C* = 0` rows, all 3 achromatic rows recovered, `height_m` exact, `vegetation` to 0.0070. ⚠️ It **cannot re-base the old table** — the lost script's sampling is unrecoverable, so every building moves ~1.2 `Δab` and `Q26`, `Q30`, `Q34` need re-grading. Rerunning the other five sheets is the decision |
...
`Q25` `Q27` `Q28` `Q29` `Q32` `Q33` `Q34` `Q36`. Each has a record in `DECISIONS.md`.
