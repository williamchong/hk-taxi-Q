...
| `Q40` | **Can façade grammar be surveyed instead of hashed?** | `Q26`'s objection to `A` is that it abandons the accurate surface — a surveyed grammar would have dissolved it | `P3-9a` | 🟡 Open, **narrowed**. 🔥 **Grammar killed by the validation sheet — 1 right, 8 wrong, 3 unsure over 12 graded faces.** `L*` modulation measures materials, not recession, so a dark-glass curtain wall reads "punched" at `L*` 17.4; glass reflections are most of the signal; one lattice screen reads `fin` from W and `curtain` from S. `Q26`'s objection to `A` therefore **stands**. ✅ The **world-space unwrap works** and atlas-space is dead (5,831 charts, only **15.2%** analysable). ✅ Glazed-vs-blank and a 2-D `L*`×`b*` tint survive — they rest on a bimodality dip, not periodicity — behind a **resolution gate**. ⚠️ **17 of 56 buildings measure a warm glass the three authored tints cannot express.** ✅ **The contamination check is discharged** (`tools/facade_glazing.py`, committed): one dip statistic from both texel selections moves the verdict on **19 of 51** gated buildings (net toward unimodal, median dip 0.490 → 0.611), and the blank block the reader refused reads *bimodal* only through the contaminated selection — so Probe 1's numbers describe an uncommitted statistic over a contaminated selection, and the glazing gate's thresholds **re-derive from the decontaminated selection** when the survey extension is built. The second sheet agrees: `11-SW-14B` moves **195 of 699** gated verdicts, in *both* directions (102 toward unimodal, 93 toward bimodal), and the two sheets' populations differ (51% vs 73% clearly unimodal, decontaminated) — the re-derivation needs both. ⚠️ `Q41` narrows the grammar kill to *per-pixel statistics* — the data carries the signal |
