...
# Degenerate triangles are dropped rather than shipped, `signs.py`'s constant and
# its reason: a zero-area triangle has no normal, so `facing_away` cannot grade
# it and it would sit in the count forever.
...
class _Builder:
    ...
    `signs._Builder`'s shape, reduced: this layer is **one** colour, so the
    vertex colour carries nothing a material could not — it is kept because
    `signs.gdshader` reads `COLOR_0` and a mesh that stopped supplying it would
    render **white**, and because a second city names its own material.

    ⚠️ **That single colour is why `signs.colours`' exemption from `Q33` does not
    transfer to this stage**: the sign livery has to ride the vertex because a
    plate is four colours in one draw call, and a lamp post is not. It comes out
    of the city's `materials:` table instead, where `_check_exposure` grades it.
    """
...
def _lantern(builder: _Builder, spec: Lamps, centre: np.ndarray, forward: np.ndarray) -> None:
...
def _draw_lamp(builder: _Builder, spec: Lamps, post: _Placed) -> None:
