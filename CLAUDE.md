...
- **`pipeline/railings.py`, the `railings` config block, or any railing change:
  `.claude/rules/railings.md`.** It loads itself when a railing file is read; read it by hand when
  the change arrives from elsewhere — ⚠️ **a widening change is a railing change**
  (`surface.floor_default_m` moves the drawn kerb and therefore every fence).
